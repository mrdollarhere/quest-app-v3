...
export default function AdminActivityPage() {
  ...
  // REGISTRY HANDSHAKE: Fetch from both telemetry nodes
  const { data: events, isLoading: eventsLoading, mutate: mutateEvents } = useSWR('/api/proxy/admin/events?limit=2000');
  const { data: logins, isLoading: loginsLoading, mutate: mutateLogins } = useSWR('/api/proxy/admin/activity');
   ...
   * Combines and standardizes records from different schema sources.
   */
  const unifiedData = useMemo(() => {
    if (!events && !logins) return [];
    ...
    const normalizedLogins = (Array.isArray(logins) ? logins : []).map(l => ({
      ...
      user_role: 'admin', // Activity sheet entries are typically higher-role actions
      event_type: l.Event,
      context: 'Access Gate',
      ip: l['IP Address'] || 'N/A',
      device: l['Device'] || 'N/A',
  ...
  if ((eventsLoading || loginsLoading) && unifiedData.length === 0) {
    return (
      <div className="py-40">
        <AILoader messages={["Synchronizing Telemetry Nodes...", "Analyzing Forensic Timeline..."]} />
      </div>
    );
  }
